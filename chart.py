...
def symbol_chart():
    for symbol in Symbols:
        ...
        plt.plot(date, bg, color="#ff9100", label="Short", zorder=1)
        plt.fill_between(date, bg, color="#ff9100")
        plt.twinx()
        plt.xticks([])
        plt.yticks(fontsize=6)
        plt.ylim([0, 100])
        plt.plot(date, sentiment, color="#00bfa5", label="Long", zorder=3)
        plt.fill_between(date, sentiment, color="#00bfa5")
        plt.grid(linestyle="dotted", color='#777777', zorder=0)
