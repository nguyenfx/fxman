...
class Controller:
    ...
    def insert_signal(self, number, symbol, type, risk, open_price):
        ...
        cursor.execute(statement, [number, symbol, type, risk, open_price, open_price, open_price, open_price])
        db.commit()
        return True
    ...
    def calculate_contrarian(self):
        ...
        statement = "UPDATE sentiments SET contrarian = (CASE " \
                    "    WHEN (sentiment > -55 AND sentiment < -5 AND sentiment < (SELECT sentiment FROM sentiments AS sub WHERE sub.site = 'avg' AND sub.symbol = sentiments.symbol AND sub.date = DATE(sentiments.date,'-1 day'))) " \
    ...
    def get_statistic(self, number):
        db = self.get_db()
        cursor = db.cursor()
        statement = "SELECT number, substr(date, 3) AS date, ROUND(profit - 0.5) AS profit, balance, percent, growth FROM statistic WHERE number = ? " \
                    "ORDER BY date "
        cursor.execute(statement, [number])
        return cursor.fetchall()
    ...
    def get_statistic_m(self, number):
        db = self.get_db()
        cursor = db.cursor()
        statement = "SELECT number, strftime('%Y-%m', date) AS month, ROUND(SUM(profit) - 0.5) AS profit, AVG(balance), SUM(percent), " \
