...
class Controller:
    ...
    def calculate_contrarian(self):
        ...
        statement = "UPDATE sentiments SET contrarian = (CASE " \
                    "    WHEN (sentiment < -10 AND sentiment < (SELECT sentiment FROM sentiments AS sub WHERE sub.site = 'avg' AND sub.symbol = sentiments.symbol AND sub.date = DATE(sentiments.date,'-1 day'))) " \
